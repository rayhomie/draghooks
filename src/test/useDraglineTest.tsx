...
const UseDraglineTest: React.FC<PropTypes> = (props) => {
  ...
  useEffect(() => {
    ...
    Store.forEach((v, i) => {
      ...
      v.current.addEventListener("mousedown", (e: any) => {
        MouseDown(e, i);
      });
      v.current.addEventListener("mousemove", (e: any) => {
        MouseMove(e, i);
      });
      v.current.addEventListener("mouseup", (e: any) => {
        MouseUp(e, i);
      });
    });
  ...
  const MouseMove = (e: any, index: number) => {
    ...
    fastdom.mutate(() => {
      ...
      storeAllRef.current[index].current.style.left = `${
        POSITIONARR.current[index][0] + mouseX - MOUSEPOSITION.current[0]
      }px`;
      storeAllRef.current[index].current.style.top = `${
        POSITIONARR.current[index][1] + mouseY - MOUSEPOSITION.current[1]
      }px`;
    });
  };
